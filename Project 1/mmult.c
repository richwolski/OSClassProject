...
int main(int argc, char** argv){
	...
	for(i = 0; i < thread_num; i++){
		...
		//printf("firstLine is %d\n", threadArg->firstLine);
		if(nextLine >= row_a){
			//this is to signify that the matrix will be empty
			threadArg->firstLine = nextLine;
			threadArg->lastLine = nextLine -1;
		}
		else
			threadArg->lastLine = nextLine + (row_a - nextLine - 1)/(thread_num-i);
		//printf("lastLine is  %d\n", threadArg->lastLine);
	...
	for(threadID = 0; threadID < thread_num; threadID++){
		...
		for(resultRow = resultFirstLine; resultRow<= resultLastLine; resultRow++){
			int result_i = resultRow - resultFirstLine;
			for(resultColumn = 0; resultColumn < col_b; resultColumn++){
				//assert(resultRow*col_b + resultColumn >= 0);
				//assert((resultRow-resultFirstLine)*col_b + resultColumn <= row_a*col_b);
				resultMatrix[resultRow*col_b+ resultColumn] = threadResultMatrix[
					result_i*col_b +resultColumn];
			}
		}	
		free(threadResults);			
